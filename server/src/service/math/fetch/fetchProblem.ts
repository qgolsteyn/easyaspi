...
import { generateArithmeticProblem } from '../../../service/math/generate/generateProblems';
import { ProblemArchetype, ProblemType, Problem } from 'shared';
import { ObjectId } from 'bson';

export async function fetchProblem(studentId:ObjectId, difficultyTier: string): Promise<Problem> {
    ...
    if (template !== null) {
        ...
        if (typeof difficulty !== 'undefined') {
            const db_problem = generateArithmeticProblem(difficulty, template.operators);
            ...
            for (let i = 0; i < db_problem.operands.length; i++) {
                if (i !== db_problem.operands.length-1) {
                    problem = problem.concat(String(db_problem.operands[i]) + " " + db_problem.operators[0] + " ");
                } else {
                    problem = problem.concat(String(db_problem.operands[i]) + " =");
                }
            }
