        ...
        if (template.problemArchetype.valueOf() === ProblemArchetype.ARITHMETIC.valueOf()) {
            ...
            template.difficultyMap.forEach((difficulty: IArithmeticDifficulty, tier: string) => {
                if (typeof difficulty !== 'undefined') {
                    let problems = generateArithmeticProblems(difficulty, template.operators);
...
export function generateArithmeticProblem(
    ...
    for (let i = 0; i < numOperands; i++) {
        ...
        if (i < difficulty.operands.length) {
            operands[i] = generateNumber(difficulty.operands[i][0], 
                difficulty.operands[i][1]);
        } else {
            // else use the last defined operand range
            operands[i] = generateNumber(difficulty.operands[difficulty.operands.length-1][0], 
                difficulty.operands[difficulty.operands.length-1][1]);
        }
    }

    // for now we just deal with 1 operator per problem
    let result: number = findArithmeticResult(operators[0], operands[0], operands[1]);
