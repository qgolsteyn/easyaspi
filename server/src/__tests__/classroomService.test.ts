...
import { authenticateToClassroom, createClassroom, getStudents } from '../service/classroomService';


test('Check if createClassroom creates classroom correctly', async () => {
    mockingoose(ClassroomModel).toReturn({name: 'Test100', passcode: '12345', problemsForToday: []}, 'save');
    try{
        const classRoom = await createClassroom(classRoomDoc4);
        expect(JSON.parse(JSON.stringify(classRoom))).toMatchObject(classRoomDoc4);
    } catch (e) {
        expect(e).toEqual('A classroom already exists with this given name and passcode');
    }
});
...
test('Check if authenticateToClassroom validates classroom properly - Should throw an error', async () => {
    ...
    try{
        ...
        expect(err.output.payload.message).toEqual('Invalid classroom name or passcode');
    }
});
